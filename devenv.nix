...
PYTHON_SCRIPT
      ...
      opencode-dev.exec = ''
        ...
        secretspec run -- bash -lc '
          ...
          # Execute the flake app from our forked opencode (hardcoded)
          # Provide base flake ref in env for downstream scripts
          export FLAKE_REF="github:americanservices/opencode-openpipe-fork"
          ...
          # Compose the app ref explicitly when invoking nix run (avoid expanding FLAKE_REF under nounset)
          APP_REF="github:americanservices/opencode-openpipe-fork#opencode-dev"
          exec nix --extra-experimental-features "nix-command flakes" run --no-write-lock-file "$APP_REF" -- "$@"
        ' bash "$@"
      '';
